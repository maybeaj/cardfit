...
import { CtaBar, Notice, Panel, PrimaryLink, SampleBadge, ScreenHeader } from '@/components/ui'
        ...
        {d.perCard.map((card) => (
          <Panel key={card.card_id}>
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="m-0 text-[12px] text-muted">{card.issuer}</p>
            ...
            <dl className="mt-3 mb-0 grid grid-cols-3 gap-2 border-t border-line pt-3">
              <div>
                <dt className="m-0 text-[11px] text-muted">적용 적립·할인율</dt>
                <dd className="m-0 mt-0.5 text-[14px] font-bold text-ink tabular-nums">
                  {card.tier ? percent(card.tier.rate) : '—'}
                </dd>
              </div>
              <div>
                <dt className="m-0 text-[11px] text-muted">월 한도</dt>
                <dd className="m-0 mt-0.5 text-[14px] font-bold text-ink tabular-nums">
                  {card.tier ? won(card.tier.monthly_cap) : '—'}
                </dd>
              </div>
              <div>
                <dt className="m-0 text-[11px] text-muted">연회비</dt>
                <dd className="m-0 mt-0.5 text-[14px] font-bold text-ink tabular-nums">
                  {won(card.annual_fee)}
                </dd>
              </div>
            ...
            {card.tierIsLowest ? (
              <p className="mt-3 mb-0 rounded-lg bg-[#FFF8E8] px-3 py-2 text-[12px] text-warning">
